...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const images = ranking.hasPreviewImage ? [{ url: `https://rankly.etienneschwab.ch/api/rankings/${ranking.slug}/preview`, width: 1200, height: 630, alt: ranking.title }] : [];
  return {
    title: `${ranking.title} | Rankly`, description,
    openGraph: { title: ranking.title, description, images },
    twitter: { card: images.length ? 'summary_large_image' : 'summary', title: ranking.title, description, images },
  };
}
