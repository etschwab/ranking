...
import { ArrowRight, BarChart3, Check, Clapperboard, Link2, ListChecks, LogIn, MapPinned, Plus, Save, Share2, Sparkles, Trophy, Utensils, UserRound, WandSparkles } from 'lucide-react';
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (storedDraft) {
        const draft = JSON.parse(storedDraft) as { title?: string; description?: string; options?: string };
        if (draft.title || draft.description || draft.options) {
  ...
  useEffect(() => {
    ...
    localStorage.setItem('rankly-creator-draft', JSON.stringify({ title, description, options }));
  }, [description, draftReady, options, title]);

  function applyPreset(preset: (typeof creatorPresets)[number]) {
    setTitle(preset.title);
    setDescription(preset.description);
    setOptions(preset.options.join('\n'));
    document.querySelector('#ranking-title')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
  }
      ...
      <section id="erstellen" className="border-y-[3px] border-foreground bg-[#fff5e7]/95 px-5 py-20 sm:px-8 lg:py-24">
        <div className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[0.7fr_1.3fr]">
          ...
          <form className="grid gap-5 rounded-[1.5rem] border-[3px] border-foreground bg-card p-5 shadow-[8px_8px_0_var(--ink)] sm:p-7" onSubmit={createRanking}>
            <div className="rounded-xl border-2 border-foreground bg-[#f7f3eb] p-4"><div className="flex items-center gap-2"><WandSparkles className="size-5 text-primary" /><p className="text-sm font-black">Schnell starten mit einer Vorlage</p></div><div className="mt-3 flex flex-wrap gap-2">{creatorPresets.map((preset) => <button key={preset.label} type="button" onClick={() => applyPreset(preset)} className="inline-flex h-9 items-center gap-2 rounded-lg border-2 border-foreground bg-card px-3 text-sm font-black shadow-[2px_2px_0_var(--ink)] transition hover:-translate-y-0.5"><preset.icon className="size-4" /> {preset.label}</button>)}</div></div>
            ...
            <label className="grid gap-2 text-sm font-black" htmlFor="ranking-options"><span className="flex items-center justify-between gap-2"><span className="flex items-center gap-2"><span className="grid size-6 place-items-center rounded-full bg-foreground text-xs text-background">3</span>Optionen</span><span className={`rounded-full px-2.5 py-1 text-xs ${parsedOptions.length >= 2 ? 'bg-[#d9f7e4] text-[#125a2f]' : 'bg-muted text-muted-foreground'}`}>{parsedOptions.length}/30</span></span><Textarea id="ranking-options" required className="min-h-40 rounded-xl border-2 border-foreground px-4 py-3 text-base leading-relaxed" value={options} onChange={(event) => setOptions(event.target.value)} /></label>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <p className="flex items-center gap-2 text-sm font-semibold text-muted-foreground"><Save className="size-4" /> Entwurf wird automatisch gespeichert</p>
