import { BarChart3, Clock3, LockKeyhole, LogOut, Pencil, Plus, UserRound, Users } from 'lucide-react';
    ...
    <main className="rankly-page min-h-screen pb-24">
      ...
      <section className="mx-auto max-w-6xl px-5 pt-10 sm:px-8">
        ...
        {rankings.length === 0 ? <div className="mt-12 rounded-[1.75rem] border-[3px] border-foreground bg-card p-10 text-center shadow-[7px_7px_0_var(--ink)]"><h2 className="text-3xl font-black">Noch nichts gerankt</h2><p className="mt-2 font-medium text-muted-foreground">Erstelle dein erstes Ranking und teile es mit deiner Gruppe.</p></div> :
          <div className="mt-12 grid gap-5 md:grid-cols-2">{rankings.map((ranking, index) => { const closed = ranking.closesAt !== null && Date.now() >= ranking.closesAt; return <article key={ranking.slug} className="rankly-card overflow-hidden rounded-[1.5rem] border-[3px] border-foreground bg-card shadow-[6px_6px_0_var(--ink)]"><div className="h-3 border-b-2 border-foreground" style={{ background: ['var(--tier-s)', 'var(--tier-a)', 'var(--tier-c)', '#d9cffd'][index % 4] }} /><div className="p-5 sm:p-6"><div className="flex items-start justify-between gap-3"><div><h2 className="text-2xl font-black tracking-tight">{ranking.title}</h2>{ranking.description && <p className="mt-2 line-clamp-2 font-medium text-muted-foreground">{ranking.description}</p>}</div><span className="flex shrink-0 items-center gap-1.5 rounded-full border-2 border-foreground bg-[#fff1a8] px-3 py-1 text-sm font-black"><Users className="size-4" /> {ranking.ballotCount}</span></div>{ranking.closesAt && <p className={`mt-4 flex items-center gap-2 text-sm font-black ${closed ? 'text-[#8a1717]' : 'text-muted-foreground'}`}>{closed ? <LockKeyhole className="size-4" /> : <Clock3 className="size-4" />}{closed ? 'Abstimmung geschlossen' : `Offen bis ${new Date(ranking.closesAt).toLocaleString('de-CH', { dateStyle: 'medium', timeStyle: 'short' })}`}</p>}<div className="mt-6 flex flex-wrap gap-2"><a href={`/r/${ranking.slug}/results`} className="inline-flex h-9 items-center gap-2 rounded-lg bg-primary px-3 text-sm font-black text-primary-foreground"><BarChart3 className="size-4" /> Auswertung</a><a href={`/r/${ranking.slug}/edit`} className="inline-flex h-9 items-center gap-2 rounded-lg border-2 border-foreground bg-card px-3 text-sm font-black"><Pencil className="size-4" /> Bearbeiten</a><CopyLinkButton path={`/r/${ranking.slug}`} /></div></div></article>; })}</div>}
      </section>
    </main>
  );
}
