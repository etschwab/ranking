...
export async function POST(request: Request, { params }: RouteContext) {
  try {
    ...
    return Response.json(
      ...
      {
        headers: {
          'Set-Cookie': accessCookie(request, slug, access.accessToken),
        },
      },
    );
  } catch {
    return Response.json(
      { error: 'Der Zugriff konnte nicht freigeschaltet werden.' },
      { status: 500 },
    );
  }
}
