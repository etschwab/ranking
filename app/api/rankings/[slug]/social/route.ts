...
export async function GET(request: Request, { params }: RouteContext) {
  ...
  if (!social) return Response.json({ error: 'Ranking nicht gefunden.' }, { status: 404 });
  return Response.json({ ...social, signedIn: Boolean(user), signInPath: chatGPTSignInPath(`/r/${slug}`) });
}
