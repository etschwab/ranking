...
type NormalizedEntry = readonly [itemId: string, tier: number, rankPosition: number];
...
export async function POST(request: Request, { params }: RouteContext) {
  ...
  try {
    ...
    if (!normalizedEntries) return Response.json({ error: 'Bitte ordne jede Option einer Stufe zu.' }, { status: 400 });

    await ensureSchema();
    const voterName = ranking.nameMode === 'required' ? (await getUserProfile(user))?.displayName ?? 'Anonym' : 'Anonym';
    const requestedToken = typeof body.editToken === 'string' ? body.editToken.trim() : '';

    const result = await upsertBallot({ ranking, userId: user.userId, voterName, requestedToken, normalizedEntries });
    if (!result) return Response.json({ error: 'Diese Abstimmung kann nicht bearbeitet werden.' }, { status: 403 });
    return Response.json({ ok: true, editToken: result.editToken, updated: result.updated }, { status: result.updated ? 200 : 201 });
  } catch (error) {
...
function normalizeBallotEntries(ranking: RankingData, body: { scores?: unknown; orders?: unknown }): NormalizedEntry[] | null {
  ...
  const orderedEntries = entries.map(([itemId, tier]) => ({
    itemId,
    tier: Number(tier),
    requestedOrder: Number.isInteger(orders[itemId]) && Number(orders[itemId]) >= 0 ? Number(orders[itemId]) : 0,
  }));

  return orderedEntries.map((entry) => {
    const rankPosition = orderedEntries
      .filter((candidate) => candidate.tier === entry.tier)
      .sort((a, b) => a.requestedOrder - b.requestedOrder || (itemPositions.get(a.itemId) ?? 0) - (itemPositions.get(b.itemId) ?? 0))
...
async function findExistingBallotId(ranking: RankingData, userId: string, requestedToken: string): Promise<string | null> {
  if (ranking.oneVotePerUser) {
    const existing = await db.prepare('SELECT id FROM ballots WHERE ranking_id = ? AND user_id = ? ORDER BY created_at DESC LIMIT 1').bind(ranking.id, userId).first<{ id: string }>();
...
async function overwriteBallot(ballotId: string, userId: string, voterName: string, normalizedEntries: NormalizedEntry[]) {
  const tokenRow = await db.prepare('SELECT token FROM ballot_edit_tokens WHERE ballot_id = ?').bind(ballotId).first<{ token: string }>();
  await db.prepare('DELETE FROM scores WHERE ballot_id = ?').bind(ballotId).run();
  await db.batch([
    db.prepare('UPDATE ballots SET voter_name = ?, user_id = ?, created_at = ? WHERE id = ?').bind(voterName, userId, Date.now(), ballotId),
...
async function insertBallot(rankingId: string, userId: string, voterName: string, normalizedEntries: NormalizedEntry[]) {
  const ballotId = crypto.randomUUID();
  const editToken = `${crypto.randomUUID()}${crypto.randomUUID()}`;
  await db.batch([
    db.prepare('INSERT INTO ballots (id, ranking_id, voter_name, user_id, created_at) VALUES (?, ?, ?, ?, ?)').bind(ballotId, rankingId, voterName, userId, Date.now()),
 ...
 */
async function upsertBallot({ ranking, userId, voterName, requestedToken, normalizedEntries }: UpsertBallotArgs) {
  const existingBallotId = await findExistingBallotId(ranking, userId, requestedToken);

  if (existingBallotId) {
    const editToken = await overwriteBallot(existingBallotId, userId, voterName, normalizedEntries);
    return { editToken: editToken ?? requestedToken, updated: true };
  }

  if (requestedToken) return null;

  const editToken = await insertBallot(ranking.id, userId, voterName, normalizedEntries);
