...
function loginRedirect(
  ...
  const headers = new Headers({
    Location: new URL(`/login?${params}`, request.url).toString(),
  });
...
export async function GET(request: Request) {
  ...
  const nextPath = safeReturnPath(
    decodeURIComponent(cookieValue(request, SSO_NEXT_COOKIE) || '/'),
  );
  ...
  if (!config)
    return loginRedirect(
      request,
      nextPath,
      'Die zentrale Anmeldung ist für diese Umgebung noch nicht eingerichtet.',
      secure,
    );
  ...
  if (oauthError) {
    return loginRedirect(
      ...
      oauthError === 'access_denied'
        ...
        : 'Die zentrale Anmeldung ist fehlgeschlagen.',
      secure,
    );
  }
  ...
  if (
    ...
    return loginRedirect(
      request,
      nextPath,
      'Die Sicherheitsprüfung der Anmeldung ist fehlgeschlagen. Bitte versuche es erneut.',
      secure,
    );
  }
