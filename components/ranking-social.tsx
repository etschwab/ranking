...
function ReactionButtons({
  ...
  return (
    <div
      ...
      aria-label={`Reaktionen für ${label}`}
    >
      {reactionEmojis.map((emoji) => {
        ...
        return (
          <button
            ...
            aria-label={`${emoji} auf ${label}${state.count ? `, ${state.count} Reaktionen` : ''}`}
            disabled={disabled}
            onClick={() => onReact(emoji)}
            className={`inline-flex h-9 min-w-10 items-center justify-center gap-1 rounded-lg border-2 px-2 text-sm font-black transition hover:-translate-y-0.5 disabled:cursor-wait disabled:opacity-60 ${state.reacted ? 'border-foreground bg-[#d9cffd] shadow-[2px_2px_0_var(--ink)]' : 'border-foreground/25 bg-card hover:border-foreground'}`}
          >
...
export function RankingSocial({
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/rankings/${slug}/social`, {
        cache: 'no-store',
      });
      const data = (await response.json()) as SocialResponse;
      if (!response.ok)
        throw new Error(
          data.error ?? 'Reaktionen konnten nicht geladen werden.',
        );
      setSocial(data);
      setError('');
    } catch (reason) {
      setError(
        reason instanceof Error
          ? reason.message
          : 'Reaktionen konnten nicht geladen werden.',
      );
    } finally {
      setLoading(false);
    }
  ...
  async function react(
    ...
    setError('');
    try {
      const response = await fetch(`/api/rankings/${slug}/reactions`, {
        method: 'POST',
      ...
      if (!response.ok)
        throw new Error(
          data.error ?? 'Reaktion konnte nicht gespeichert werden.',
        );
      await load();
    } catch (reason) {
      setError(
    ...
    <section className="mt-10 rounded-[1.75rem] border-[3px] border-foreground bg-[#fff5e7] p-5 shadow-[7px_7px_0_var(--ink)] sm:p-7">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="flex items-center gap-2 text-sm font-black uppercase tracking-[0.14em] text-primary">
            <SmilePlus className="size-5" /> Reaktionen
          </p>
          <h2 className="mt-1 text-3xl font-black tracking-tight">
            Was fühlt die Runde?
          </h2>
          <p className="mt-1 font-semibold text-muted-foreground">
            Reagiere auf Optionen und diskutiere gemeinsam in den Kommentaren.
          </p>
        </div>
        <button
          type="button"
          onClick={() => void load()}
          disabled={loading}
          className="grid size-10 place-items-center rounded-xl border-2 border-foreground bg-card"
          aria-label="Reaktionen aktualisieren"
        >
      ...
      <div className="mt-6 grid gap-3 md:grid-cols-2">
        {items.map((item) => (
          <article
            key={item.id}
            className="rounded-xl border-2 border-foreground bg-card p-4"
          >
            <h3 className="mb-3 font-black">{item.label}</h3>
            <ReactionButtons
              label={item.label}
              reactions={social?.itemReactions[item.id] ?? []}
              disabled={pending.startsWith(`item:${item.id}:`)}
              onReact={(emoji) => void react('item', item.id, emoji)}
            />
          </article>
        ))}
      </div>
      ...
      <div className="mt-8 border-t-[3px] border-foreground pt-7">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-black">Kommentare</h2>
            <p className="text-sm font-semibold text-muted-foreground">
              {social?.comments.length ?? 0} Beiträge
            </p>
          </div>
        </div>
        {social?.signedIn ? (
          <form onSubmit={submitComment} className="mt-5 grid gap-3">
            <Textarea
              ...
              maxLength={500}
              required
              placeholder="Was denkst du über dieses Ranking?"
              className="min-h-24 border-2 border-foreground bg-card px-4 py-3"
            />
            <div className="flex items-center justify-between gap-3">
              ...
              >
                {pending === 'comment' ? 'Wird gesendet…' : 'Kommentieren'}{' '}
                <Send />
              </Button>
            </div>
          </form>
          ...
          <a
            href={
              social?.signInPath ??
              `/login?returnTo=${encodeURIComponent(`/r/${slug}`)}`
            }
            className="mt-5 inline-flex h-11 items-center gap-2 rounded-xl border-2 border-foreground bg-primary px-5 font-black text-primary-foreground shadow-[3px_3px_0_var(--ink)]"
          >
            <LogIn className="size-4" /> Zum Reagieren anmelden
          </a>
        )}

        <div className="mt-6 space-y-3">
          {social?.comments.map((entry) => (
            <article
              key={entry.id}
              className="rounded-xl border-2 border-foreground bg-card p-4"
            >
              <div className="flex flex-wrap items-center justify-between gap-2">
                <p className="font-black">{entry.authorName}</p>
                <time
          ...
          {!loading && social?.comments.length === 0 && (
            <div className="rounded-xl border-2 border-dashed border-foreground/35 bg-card/70 p-6 text-center font-semibold text-muted-foreground">
