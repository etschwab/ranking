  ...
  return (
    <form onSubmit={submit} className="mt-10 grid gap-7">
      ...
      <section className="rounded-[1.5rem] border-[3px] border-foreground bg-card p-5 shadow-[6px_6px_0_var(--ink)] sm:p-7">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div className="flex gap-3"><span className="grid size-11 shrink-0 place-items-center rounded-xl border-2 border-foreground bg-[#d9cffd]"><CopyPlus className="size-5" /></span><div><h2 className="text-xl font-black">Als Vorlage verwenden</h2><p className="mt-1 text-sm font-semibold text-muted-foreground">Erstellt mit einem Klick eine bearbeitbare Kopie ohne Stimmen und Einsendeschluss.</p></div></div><Button type="button" variant="outline" onClick={duplicateRanking} disabled={duplicating} className="h-11 border-2 border-foreground font-black"><CopyPlus /> {duplicating ? 'Wird erstellt…' : 'Vorlage erstellen'}</Button></div>
      </section>
