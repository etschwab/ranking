...
export function RankingShare({ slug, title }: { slug: string; title: string }) {
  ...
  useEffect(() => {
    ...
    void QRCode.toDataURL(shareUrl, {
      width: 360,
      margin: 2,
      color: { dark: '#241d1a', light: '#ffffff' },
    ...
    <section className="mt-10 rounded-[1.5rem] border-[3px] border-foreground bg-[#d9cffd] p-5 shadow-[6px_6px_0_var(--ink)] sm:p-6">
      <div className="grid gap-5 sm:grid-cols-[132px_1fr] sm:items-center">
        ...
        <div>
          ...
          <h2 className="mt-1 text-2xl font-black">QR-Code oder WhatsApp</h2>
          <p className="mt-1 break-all text-sm font-semibold text-muted-foreground">
            {url || `rankly.etienneschwab.ch/r/${slug}`}
          </p>
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex h-11 items-center gap-2 rounded-lg border-2 border-[#145f32] bg-[#25d366] px-4 font-black text-[#103e23] shadow-[2px_2px_0_#145f32]"
            >
