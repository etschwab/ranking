...
export async function registerWithEmail(
  ...
  await db.batch([
    db
      ...
      .bind(userId, normalizedEmail, passwordHash, displayName, Date.now()),
    db
      .prepare(
        'INSERT INTO user_profiles (user_id, display_name, email, updated_at) VALUES (?, ?, ?, ?)',
      )
      .bind(userId, displayName, normalizedEmail, Date.now()),
  ]);
  return userId;
}
...
export async function findOrCreateGoogleUser(input: {
  ...
  const existing = await db
    .prepare(
      'SELECT id, email, display_name AS displayName, password_hash AS passwordHash, google_sub AS googleSub FROM users WHERE google_sub = ? OR email = ? ORDER BY CASE WHEN google_sub = ? THEN 0 ELSE 1 END LIMIT 1',
    )
  ...
  await db.batch([
    db
      ...
      .bind(userId, email, input.sub, input.displayName, Date.now()),
    db
      .prepare(
        'INSERT INTO user_profiles (user_id, display_name, email, updated_at) VALUES (?, ?, ?, ?)',
      )
      .bind(userId, input.displayName, email, Date.now()),
  ]);
  return userId;
}
...
export async function findOrCreateSsoUser(input: {
  ...
  if (existing) {
    await db.batch([
      db
        .prepare('UPDATE users SET esch_sub = ?, display_name = ? WHERE id = ?')
        .bind(
          input.sub,
          existing.displayName || input.displayName,
          existing.id,
  ...
  await db.batch([
    db
      .prepare(
        'INSERT INTO users (id, email, password_hash, google_sub, esch_sub, display_name, created_at) VALUES (?, ?, NULL, NULL, ?, ?, ?)',
      )
      .bind(userId, email, input.sub, input.displayName, Date.now()),
    db
      .prepare(
...
export async function migrateLegacyAccount(
  ...
  await db.batch([
    db
      .prepare(
        'UPDATE ranking_owners SET user_id = ?, email = ? WHERE user_id = ?',
      )
      .bind(userId, email, legacyUserId),
    db
      .prepare('UPDATE ballots SET user_id = ? WHERE user_id = ?')
